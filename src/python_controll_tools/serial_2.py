...
class SerialGUI:
    ...
    def create_sliders(self, slider_names):
        ...
        for i, slider_name in enumerate(slider_names):
            ...
            label.grid(row=5, column=i * 2, pady=2, padx=2)

            var = tk.DoubleVar()
            slider = ttk.Scale(self.frame, from_=0, to=100, orient=tk.HORIZONTAL, variable=var, command=self.parse_gui)
            slider.grid(row=5, column=i * 2 + 1, pady=2, padx=2)
